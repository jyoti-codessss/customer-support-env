...
DEMO_SCRIPTS = {
    "billing_dispute_easy": [
        "Thank you for contacting support. Let me pull up your account details.",
        "I can see the duplicate charge. I sincerely apologize for this inconvenience.",
        "I am processing a full refund. You will see it within 3-5 business days.",
        "Is there anything else I can help you with today?",
    ],
    "technical_outage_medium": [
        "I understand you are experiencing difficulties. I am here to help.",
        "I have checked our system and identified the issue affecting your service.",
        "I am escalating this with high priority. Here is a workaround you can use.",
        "The issue is resolved. I am adding a service credit to your account.",
    ],
    "fraud_complaint_hard": [
        "I take fraud reports seriously. Let me secure your account immediately.",
        "I have frozen suspicious activity. Please confirm unauthorized transactions.",
        "I have filed a fraud report and reversed the charges. New credentials issued.",
        "Your account is secured. You will receive an investigation report in 24 hours.",
    ],
    "subscription_cancellation_hard": [
        "I am sorry to hear you want to cancel. May I ask what prompted this?",
        "I understand your frustration. Let me see what I can offer.",
        "I would like to offer you 3 months free plus a plan upgrade.",
        "I have processed your request. Account active until end of billing period.",
    ],
    "vip_account_recovery_expert": [
        "As a VIP customer, you have my personal attention. Initiating recovery.",
        "I have verified your identity through secure VIP verification process.",
        "I am restoring full access with enhanced security measures. Data intact.",
        "Your VIP account is fully restored. A dedicated manager will contact you shortly.",
    ],
}
...
HEADER = (
    '<div style="text-align:center;padding:20px;background:linear-gradient(135deg,#1e1b4b,#312e81);border-radius:16px;margin-bottom:16px;">'
...
def on_task_select(task_id):
    ...
    info_html = (
        ...
        f'<span style="color:{c};font-weight:bold;">{diff}</span>'
        f'<span style="color:#94a3b8;margin-left:8px;">{desc}</span>'
        '</div>'
    )
    msg_html = (
        ...
        f'<p style="color:#f1f5f9;margin:0;font-style:italic;">{msg}</p>'
        '</div>'
    )
...
def run_agent(task_id):
    ...
    steps_html = (
        ...
        f'<h3 style="color:#a5b4fc;margin:0 0 12px;">Agent Simulation</h3>'
        '<div style="background:#0f172a;border-left:4px solid #f59e0b;padding:12px;border-radius:8px;margin-bottom:12px;">'
        '<p style="color:#fbbf24;font-size:0.8em;margin:0 0 4px;">CUSTOMER</p>'
        f'<p style="color:#f1f5f9;margin:0;font-style:italic;">{customer_msg}</p>'
        '</div>'
    )
    for i, (resp, rew, layer) in enumerate(zip(scripts, rewards, layers), 1):
        ...
        steps_html += (
            ...
            f'<span style="color:#818cf8;font-weight:bold;">Step {i} - {layer}</span>'
            f'<span style="color:{rc};font-weight:bold;">+{rew:.2f}</span>'
            '</div>'
            f'<p style="color:#e2e8f0;margin:0;">{resp}</p>'
            '</div>'
        )
    steps_html += '</div>'

    row_style = "display:flex;justify-content:space-between;align-items:center;padding:10px 12px;border-bottom:1px solid #334155;"
    label_style = "color:#e2e8f0;font-size:0.95em;"
    val_style = "color:#34d399;font-weight:bold;font-size:0.95em;"
    ...
    score_html = (
        ...
        f'<div style="{row_style}"><span style="{label_style}">JSON Format</span><span style="{val_style}">1.00</span></div>'
        f'<div style="{row_style}"><span style="{label_style}">Empathy</span><span style="{val_style}">1.00</span></div>'
        f'<div style="{row_style}"><span style="{label_style}">Action Taken</span><span style="{val_style}">1.00</span></div>'
        f'<div style="{row_style}"><span style="{label_style}">Resolution</span><span style="{val_style}">1.00</span></div>'
        '<div style="display:flex;justify-content:space-between;align-items:center;padding:12px;background:#1e3a2e;border-top:2px solid #34d399;">'
        '<span style="color:#f1f5f9;font-weight:bold;font-size:1.05em;">Final Score</span>'
    ...
    bars = ax.bar(["Baseline\n(Untrained)", "Trained\n(GRPO)"], [0.268, 1.000],
                  color=["#f87171", "#34d399"], width=0.5, edgecolor="#334155")
    for bar, val in zip(bars, [0.268, 1.000]):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.02,
                f"{val:.3f}", ha="center", va="bottom", color="#f1f5f9", fontweight="bold", fontsize=11)
    ax.set_ylim(0, 1.25)
...
with gr.Blocks(title="CustomerSupportEnv Demo") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            task_dd = gr.Dropdown(choices=TASK_IDS, label="Select Task",
                                  info="Choose a customer support scenario", interactive=True)
